...
int bind_data(t_gpu *gpu)
{
    ...
    gpu->kernel = clCreateKernel(gpu->program, "vadd", &gpu->err);
    ...
    while (++j < WIN_H)
	{
		...
		while (++i < WIN_W)	
		{
            ...
            h_a[j * WIN_W + i] = ft_vec3_normalize((t_vec3){x, y, -1});
    ...
    gpu->err = clSetKernelArg(gpu->kernel, 1, sizeof(cl_mem), &d_a);
    ...
    // if (print_error(gpu) != 0)
    //     return 1;
    gpu->err |= clSetKernelArg(gpu->kernel, 2, sizeof(cl_mem), &out_vec);
